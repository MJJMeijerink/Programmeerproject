...
De map 'data' bevat alle data, zowel in .csv formaat als in het uiteindelijke .JSON formaat. Het python script dat gebruikt is om het .JSON bestand te maken zit ook in de map. Let wel op, de JSON die door process.py gegenereerd wordt is daarna nog een klein beetje met de hand aangepast en kan dus bugs veroorzaken als deze uitgevoerd wordt.
